...
The maximum size of a single vector element in bits, latexmath:[ELEN].
Must be a power of 2.

The number of bits in a vector register, latexmath:[VLEN \geq ELEN].  Must
be a power of 2.

NOTE: Platform profiles may set further constraints on these
parameters, for example, requiring that latexmath:[ELEN \geq
max(XLEN,FLEN)] or requiring a minimum latexmath:[VLEN] value.
